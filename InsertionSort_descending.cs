...
{
    class InsertionSort_descending : ISortEngine
    {
        ...
        public InsertionSort_descending(int[] elemnts, Graphics graphic, int maximumValue)
        {
            Elements = elemnts;
        ...
        public void algorithm()
        {
            for (int i = 1; i < Elements.Count(); ++i)
            {
                ...
                while (j >= 0 && Elements[j] < key)
                {
                    ...
                    j -= 1;
                    DrawBar(i, Elements[i]);
                    DrawBar(j + 1, Elements[j + 1]);
                }
                Elements[j + 1] = key;
...
#pragma warning disable IDE0060 
        private void DrawBar(int placing, int H)
